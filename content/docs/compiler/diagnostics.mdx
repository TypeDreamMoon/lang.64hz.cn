...
VSCode 扩展 `1.2.26` 起，`UE.CollectionParam(Collection=Path(...), ...)` 中的 `Path(...)` 会被当作 DreamShader helper 处理，不再误报 `Unknown function 'Path'`。扩展 `1.4.0` 起，本地诊断改由新的 parser-based language core 提供，能更稳定地区分 `Inputs`、`Graph`、`Function`、`Settings`、`Outputs` 和 metadata context。
