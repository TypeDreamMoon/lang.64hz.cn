---
title: 当前限制
description: DreamShaderLang 1.2.3 的边界、兼容性和推荐替代写法。
---
...
| 区域 | 支持重点 |
| --- | --- |
| `Properties` | 标量、向量、纹理、显式 Parameter 节点、`StaticSwitchParameter`、`UE.CollectionParam` 和 `UE.*`。 |
| `Graph` | 标量、向量、纹理对象引用。 |
| `ShaderFunction` / `VirtualFunction` pin | 标量、向量、纹理，可用 `opt` 表达预览默认值。 |
