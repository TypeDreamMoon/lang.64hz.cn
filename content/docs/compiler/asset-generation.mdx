...
| 文件 | 生成行为 |
| --- | --- |
| `.dsm` | 可以生成 `UMaterial` 和 `UMaterialFunction`，也可以声明 `VirtualFunction`。 |
...
`Properties`、`ShaderFunction` / `VirtualFunction` pin 声明尾部的 `[Group="...", SortPriority=32, Description="..."]` 会尽量写入对应 Unreal 节点。材质函数 pin 当前主要使用 description 和 sort priority；Unreal Function Input / Output 本身没有分组字段。

`ShaderFunction` / `VirtualFunction` 输入前的 `opt` 会映射到 FunctionInput 的“使用预览值作为默认值”语义。调用侧传入 `default` 或省略尾部可选输入时，生成器不会连接该输入 pin，让 Unreal 使用预览默认值。
