...
当前扩展版本：`1.2.22`。版本记录见 [VSCode 扩展 ChangeLog](../changelog/vscode-extension.mdx)。
...
| 补全 | section、类型、变量、函数、namespace、`UE.*`、`Path(...)`、Root / Path 插件名、import 路径。 |
...
| `Properties` | 显式 Parameter 节点、`StaticSwitchParameter`、声明元数据、`UE.CollectionParam(...)`。 |
...
| 参数元数据错误 | `SortPriority` 不是整数或元数据键不支持。 |
| `Path(...)` 错误 | 纹理默认资产路径不合法。 |
