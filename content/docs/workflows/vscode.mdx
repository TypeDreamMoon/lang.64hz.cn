...
当前扩展版本：`1.4.0`。版本记录见 [VSCode 扩展 ChangeLog](../changelog/vscode-extension.mdx)。

`1.4.0` 版本重写了语言服务核心。补全、诊断、语义高亮、折叠和文档符号现在通过 scanner / parser / context / symbol / provider 模块处理，减少不同 section 和作用域之间的误判。
...
| 补全 | section、类型、变量、函数、namespace、`UE.*`、动态 MaterialExpression、`Path(...)`、Root / Path 插件名、import 路径、`MaterialAttributes` 成员。 |
