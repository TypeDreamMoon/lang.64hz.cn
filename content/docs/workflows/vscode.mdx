...
当前扩展版本：`1.2.24`。版本记录见 [VSCode 扩展 ChangeLog](../changelog/vscode-extension.mdx)。
...
| `ShaderFunction` / `VirtualFunction` | 顶层声明、`Options.Asset`、输入输出 pin、`opt` / `default`、Graph 调用和缺失项诊断。 |
| `Properties` | 显式 Parameter 节点、`StaticSwitchParameter`、分号式反射属性块、`UE.CollectionParam(...)`。 |
