...
纹理值可以作为 `Properties`，也可以作为 `Function` / `ShaderFunction` 参数。`ShaderFunction.Properties` 中的 `const Texture2D PreviewTex;` 会生成 Texture Object helper，可作为 `Inputs` 的预览默认值。`Graph if` 不能在两条分支之间选择纹理对象。
