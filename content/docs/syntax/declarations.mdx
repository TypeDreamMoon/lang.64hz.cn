---
title: 顶层声明
description: Shader、ShaderFunction、Function 和 Namespace 的完整结构。
---
...
| `Root` | 可选，默认 `Game`。支持 `Game`、`Plugin.PluginName`，也可追加子目录，例如 `Game/Generated`；`Plugin.PluginName` 指向 `[Project]/Plugins/PluginName/Content`。 |
