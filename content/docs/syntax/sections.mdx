...
```c
VectorParameter Tint = float4(1.0, 0.9, 0.8, 1.0) [Group="Surface", SortPriority=20, Description="Base tint"];
```
...
Inputs = {
    vec3 InColor;
    opt vec3 InTint = vec3(1.0, 1.0, 1.0) [Description="Preview tint"];
    opt float Strength = 1.0 [SortPriority=10];
}
```
...
`Inputs` / `Outputs` 支持 `[SortPriority=..., Description="..."]`。`Group` 也会被解析并保留在语法层，但 Unreal Function Input / Output 节点本身没有分组字段。
...
| --- | --- | --- | --- |
| `Properties` | 支持 | 可作为 `Inputs` 兼容写法 | 可作为 `Inputs` 兼容写法 |
| `Inputs` | 不支持 | 支持 | 支持 |
