...
当前一个 `.dsm` 中只建议放一个主要生成资产声明，例如一个 `Shader`、`ShaderFunction`、`ShaderLayer` 或 `ShaderLayerBlend`。同一个文件可以同时声明共享 helper，但从维护性看，建议按资产拆文件。
...
`.dsh` 不直接生成 `Shader` / `ShaderFunction` 资产。保存 `.dsh` 后，DreamShader 会根据 import graph 找到依赖它的 `.dsm` 并重编。
